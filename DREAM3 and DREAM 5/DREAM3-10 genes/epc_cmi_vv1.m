...
while (~stop)
    stepflag = stepflag + 1;
    %logical vectors of adjececency nodes翻译为邻接节点的逻辑向量
    ...
    %vectors翻译为从邻接节点向量中排除节点本身
    indx = cellfun(@(x, r, c) indxexclude(x, r, c), indx, num2cell(edgerow), num2cell(edgecol), 'UniformOutput', 0);
    %find the non-all-zero logical vector翻译为找到非全零逻辑向量
    indx2 = cellfun(@(x) any(x), indx);
    %find the indexes of adjecency nodes翻译为查找邻接节点的索引
